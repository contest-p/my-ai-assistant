...
5. "이번 달 얼마 썼어? / 평소보다 많아?" 같은 질문에는 current_month와 trend를 함께
   ...
   거래월(`{current_month.month}`) 기준입니다 — 답변에 실제 월을 자연스럽게 명시하세요**
...
- 가능한 범위에서 비교 근거를 제시
- 답변에 실제 월(`current_month.month`)을 명시 — 실시간 정보인 것처럼 숨기지 않음
  (시스템 프롬프트 답변 규칙 5번, 목업 검토 과정에서 발견된 요구사항)
...
```text
전역 후 1년 동안 내 소비 패턴이 어땠어? 앞으로는 어떻게 하면 좋을까?
```

기대 동작:

- 연간 총계(`total_income`/`total_expense`/`net`)와 `trend`를 근거로 패턴을 요약
- 데이터에 근거한 구체적 제안 (일반론 아님 — 시스템 프롬프트 답변 규칙 9번)
...
2026-09-14 사용자 요청: 데이터 요약의 월별 카드에서 “이번 달” 표시를 제거하고
해당 위치에 월 선택 드롭다운을 제공한다. `monthly` 전체 범위(거래 없는 중간 월 포함)를
최신순으로 표시하고 선택한 월의 수입·지출·순증감을 보여준다. 최초 선택은 최신 거래월,
요약 새로고침 시 기존 선택이 남아 있으면 유지하고 사라졌으면 최신월로 돌아간다.
로딩·실패·빈 데이터에서는 선택을 비활성화한다.
...
`OPENAI_MODEL` 환경변수로 모델명을 주입한다. 확정 모델: `gpt-5.5`(코디세이 프록시가
제공하는 모델 목록 — Claude/Gemini 계열 포함 11종 — 확인 후, 이미 구현·검증까지 끝낸
